self.onInit = function() {
        ...
        return saveEntityGroup({
            ...
            rx.switchMap((userGroup) => {
                return saveEntityGroup({
                    ...
                    rx.switchMap((entityViewGroup) => shareEntityGroupToChildOwnerUserGroup$(
                        userGroup && userGroup.id ? userGroup.id.id : null,
                        entityViewGroup && entityViewGroup.id ? entityViewGroup.id.id : null
                    ))
                );
            }),
            rx.catchError((error) => {
    ...
    function shareEntityGroupToChildOwnerUserGroup$(userGroupId, entityViewGroupId) {
        if (!userGroupId || !entityViewGroupId) {
            return rx.throwError(() => new Error('Missing group ids'));
        }
        var body = {
            entityGroupId: entityViewGroupId,
            childOwnerUserGroupId: userGroupId
        };
        return self.ctx.http.post('/api/entityGroup/shareEntityGroupToChildOwnerUserGroup', body).pipe(
            rx.catchError((error) => {
